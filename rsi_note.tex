...
\begin{abstract}
The nature of Plasma Facing Components (PFCs) in tokamaks have a strong impact on machine performance. It is therefore crucial to understand how different PFC material candidates evolve under plasma exposure in a tokamak and impact its performance. This can be achieved by probing the surfaces of PFCs using surface and material analysis techniques like X-Ray Photoelectron Spectroscopy (XPS), Sputter depth profiling, Ion Scaterring Spectroscopy (ISS) amongnst many. A particular challenge is to study and analyze lithium PFCs that are a strong PFC candidate due to its low recycling and impurity gettering properties. Li PFC analysis is complicated by the fact that lithium reacts with common residual vacuum components and plasma ions and changes relatively quickly as a function of exposure. Previous work at the Lithium Tokamak Experiment has sought to answer key questions regarding how a lithium PFC changes and impacts plasma performances using in-vacuo XPS analysis by using a small form factor XPS system that can be hooked onto the machine. We demonstrate an alternative approach that involves a vacuum suitcase that can be attached to LTX-$\beta$ and removed for analysis periodically to look at how the PFC changes with time and plasma exposure. 
\end{abstract}
